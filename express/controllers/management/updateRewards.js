    ...
    global.activeUsers.forEach(userId => {
        api.channelPoints.createCustomReward(userId, {
            ...
            TwitchReward.create({
                ...
                res.redirect("/rewards?restart&info=" + encodeURIComponent("Success! The server will be stopped to take the changes into account, and this page will refresh in a couple seconds."));
                setTimeout(() => {
                ...
                console.error(err);
                res.redirect("/rewards?error=Please+restart+the+server!");
            });
        }, err => {
            console.error(err);
            res.redirect("/rewards?error=Unable+to+create+reward!");
        });
    });
});
