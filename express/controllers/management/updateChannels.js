...
router.post("/", async (req, res) => {
    const data = req?.body;
    if (!data?.authorized) {
        return res.redirect("/users?error=At+least+one+user+must+be+authorized");
    }
    if (!data?.useAsBot) {
        return res.redirect("/users?error=One+user+must+be+used+as+bot");
    }
    if (!data?.useAsChannel) {
        return res.redirect("/users?error=One+user+must+be+used+as+channel");
    }
    ...
    res.redirect("/users?restart&info=" + encodeURIComponent("Success! The server will be stopped to take the changes into account, and this page will refresh in a couple seconds."));
    setTimeout(() => {
